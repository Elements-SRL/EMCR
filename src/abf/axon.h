...
class ABF {
    ...
    public:
        ...
        ABF_StatsRegionInfo StatsRegionSection;        // one for each stats region
        //ABF_Section MathSection;
        ABF_ScopeInfo ScopeInfo;
        char Strings[ABF_BLOCKSIZE];
        ABF_SynchArray		SynchArray[4096];
        ABF_TagsInfo        TagsInfo[4096];
    ...
    int WriteProtocolInfo(void);
    int ReadADCInfo(void);
    int WriteADCInfo(void);
    int ReadDACInfo(void);
    int WriteDACInfo(void);
    int ReadStatsRegion(void);
    int WriteStatsRegion(void);
    int ReadEpochInfo(void);
    int ReadEpochInfoPerDAC(void);
    int WriteScopeInfo(void);
    int ReadStrings(void);
    int WriteStrings(void);
    int GetString(char * str, int i);
    char * GetString(int i);
    int ReadSynchArray(void);
    int ReadAllSections(void);
    int ReadData(void * data);
    int WriteRawData(void * data, int size, int num);
    int WriteSynchData();
    int WriteTagsInfo();
    float * ReadFloatData(void);
    short * ReadIntData(void);
    int InitFileInfo(void);
    int InitProtocolSection(void);
    int InitADCSection(int);
    int InitDACSection(int);
    int InitStatsRegionSection(void);
    int InitScopeSection(void);
    int InitStrings(void);
    int PutString(const char *);
    int GetStringNumber(void);
    int GetStringsTotLen(void);
};
