...
AbfDataWriterConsumer::AbfDataWriterConsumer(ApplicationStatus * appStatus, DeviceDataProducer * producer) :
    ...
    maxDataSizeWritten = (rawBuffersLen/DWC_ABF_CHANNEL_PER_FILE)*DWC_ABF_CHANNEL_PER_FILE;
    ...
    rawVBuffers = new unsigned short * [voltageChannelsNum];
    rawVBuffers[0] = new unsigned short [rawBuffersLen*DWC_ABF_CHANNEL_PER_FILE*voltageChannelsNum];
    for (int channelIdx = 1; channelIdx < voltageChannelsNum; channelIdx++) {
        rawVBuffers[channelIdx] = rawVBuffers[channelIdx-1]+rawBuffersLen*DWC_ABF_CHANNEL_PER_FILE;
    }

    abfs.resize(currentChannelsNum);
    abfs.fill(nullptr);
    abfsV.resize(voltageChannelsNum);
    abfsV.fill(nullptr);
...
AbfDataWriterConsumer::~AbfDataWriterConsumer() {
    ...
    if (rawVBuffers[0] != nullptr) {
        delete [] rawVBuffers[0];
        rawVBuffers[0] = nullptr;
    }

    if (rawVBuffers != nullptr) {
        delete [] rawVBuffers;
        rawVBuffers = nullptr;
    }
...
long long AbfDataWriterConsumer::prepareBufferAndWriteToFile(long long &bufferIdx, double activeChannelsRatio) {
    ...
    long long rawVBufferLen;
    long long rawBufferIdx;
    long long rawVBufferCounter = 0;
    ...
    switch (settings.voltageFormat) {
    case RecordSettingsDialog::VoltageAll:
        while (bufferLen > 0) {
            rawBufferIdx = 0;
    ...
    case RecordSettingsDialog::VoltageSeparate:
        rawVBufferLen = 0;
        while (bufferLen > 0) {
            rawBufferIdx = 0;
            rawBufferLen = qMin((long long)(bufferLen/totalChannelsNum), maxDataSizeWritten);
            bufferLen -= rawBufferLen*totalChannelsNum;

            while (rawBufferIdx < rawBufferLen) {
                rawVBufferCounter++;
                if (rawVBufferCounter == voltageDecimationRatio) {
                    rawVBufferCounter = 0;
                    for (int channelIdx : activeChannels) {
                        rawVBuffers[channelIdx][rawVBufferLen] = buffer[bufferIdx+channelIdx];
                    }
                    rawVBufferLen++;
                }
            ...
            for (int channelIdx : activeChannels) {
                abfs[channelIdx]->WriteRawData(rawBuffers[channelIdx], sizeof(unsigned short), rawBufferLen);
                abfsV[channelIdx]->WriteRawData(rawVBuffers[channelIdx], sizeof(unsigned short), rawVBufferLen);
            }
        }
        break;

    case RecordSettingsDialog::VoltageNone:
        while (bufferLen > 0) {
            rawBufferIdx = 0;
            rawBufferLen = qMin((long long)(bufferLen/totalChannelsNum), maxDataSizeWritten);
            bufferLen -= rawBufferLen*totalChannelsNum;
...
void AbfDataWriterConsumer::run() {
    ...
    while (true) {
        ...
        if (hook->getDataChunk(buffer, 1, minPacketsPerBatch)) {
            ...
            long long truncatedValues = this->prepareBufferAndWriteToFile(bufferIdx, activeChannelsRatio);
...
void AbfDataWriterConsumer::initIVSections() {
    for (int channelIdx : activeChannels) {
        ABF * abf = abfs[channelIdx];
        abf->InitFileInfo();
        abf->InitStrings();
...
void AbfDataWriterConsumer::initISections() {
    for (int channelIdx : activeChannels) {
        ABF * abf = abfs[channelIdx];
        abf->InitFileInfo();
        abf->InitStrings();
        ...
        /*! ADC section */
        short adcNum = 1; /*! We only save one current in each abf file */
        abf->FileInfo.ADCSection.uBlockIndex = blockIdx;
        abf->FileInfo.ADCSection.uBytes = 128;
        abf->FileInfo.ADCSection.llNumEntries = adcNum;
        blockIdx += this->blocksUsedBySection(abf->FileInfo.ADCSection);
...
void AbfDataWriterConsumer::initVSections() {
    for (int channelIdx : activeChannels) {
        ABF * abf = abfsV[channelIdx];
        abf->InitFileInfo();
        abf->InitStrings();
        blockIdxV = 1; /*!< First block available */
        ...
        /*! Protocol section */
        abf->InitProtocolSection();
        abf->FileInfo.ProtocolSection.uBlockIndex = blockIdxV;
        abf->FileInfo.ProtocolSection.uBytes = 512;
        abf->FileInfo.ProtocolSection.llNumEntries = 1;
        blockIdxV += this->blocksUsedBySection(abf->FileInfo.ProtocolSection);

        abf->ProtocolInfo.nOperationMode = ABF_GAPFREEFILE;
        abf->ProtocolInfo.fSynchTimeUnit = 0.0;
        abf->ProtocolInfo.lNumberOfTrials = 0;

        abf->ProtocolInfo.fADCSequenceInterval = abfIntervalUsF32V;
        abf->ProtocolInfo.fADCRange = 1.0;
        ...
        /*! ADC section */
        short adcNum = 1; /*! We only save one voltage in each abf file */
        abf->FileInfo.ADCSection.uBlockIndex = blockIdxV;
        abf->FileInfo.ADCSection.uBytes = 128;
        abf->FileInfo.ADCSection.llNumEntries = adcNum;
        blockIdxV += this->blocksUsedBySection(abf->FileInfo.ADCSection);
        ...
        /*! DAC section */
        abf->FileInfo.DACSection.uBlockIndex = blockIdxV;
        abf->FileInfo.DACSection.uBytes = 256;
        abf->FileInfo.DACSection.llNumEntries = 8;
        blockIdxV += this->blocksUsedBySection(abf->FileInfo.DACSection);
        ...
        /*! Stats region section */
        abf->InitStatsRegionSection();
        abf->FileInfo.StatsRegionSection.uBlockIndex = blockIdxV;
        abf->FileInfo.StatsRegionSection.uBytes = 128;
        abf->FileInfo.StatsRegionSection.llNumEntries = 1;
        blockIdxV += this->blocksUsedBySection(abf->FileInfo.StatsRegionSection);

        /*! Strings section */
        abf->FileInfo.StringsSection.uBlockIndex = blockIdxV;
        abf->FileInfo.StringsSection.uBytes = (unsigned int)(abf->GetStringsTotLen());
        abf->FileInfo.StringsSection.llNumEntries = abf->GetStringNumber();
        blockIdxV += this->blocksUsedBySection(abf->GetStringsTotLen());

        /*! Scope section */
        abf->InitScopeSection();
        abf->FileInfo.ScopeSection.uBlockIndex = blockIdxV;
        abf->FileInfo.ScopeSection.uBytes = 769;
        abf->FileInfo.ScopeSection.llNumEntries = 1;
        blockIdxV += this->blocksUsedBySection(abf->FileInfo.ScopeSection);
        ...
        /*! Data section */
        abf->FileInfo.DataSection.uBlockIndex = blockIdxV;
        abf->FileInfo.DataSection.uBytes = 2;
        abf->FileInfo.DataSection.llNumEntries = 0; /*!< this is an initialization, this number will increase everytime new data is added */
...
void AbfDataWriterConsumer::manageConsumptionBegin() {
    ...
    voltageDecimationRatio = settings.voltageDecimationFactor;
    abfIntervalUsF32V = abfIntervalUsF32*(float)voltageDecimationRatio;
...
void AbfDataWriterConsumer::manageConsumptionEnd() {
    ...
    unsigned int blockIdxVOffset = blockIdxV;
...
//            this->writeTagsInfo(abf);
        ...
        if (settings.voltageFormat == RecordSettingsDialog::VoltageSeparate) {
            ABF * abf = abfsV[channelIdx];
            if (abf != nullptr) {
                blockIdxV = blockIdxVOffset+this->blocksUsedBySection(abf->FileInfo.DataSection);
...
void EpisodicAbfDataWriterConsumer::run() {
    this->manageConsumptionBegin();
...
void EpisodicAbfDataWriterConsumer::initAbfSections() {
    this->initIVSections();
}

void EpisodicAbfDataWriterConsumer::initIVSections() {
    for (int channelIdx : activeChannels) {
        ABF * abf = abfs[channelIdx];
        abf->InitFileInfo();
        abf->InitStrings();
...
void EpisodicAbfDataWriterConsumer::manageConsumptionBegin() {
    DataWriterConsumer::manageConsumptionBegin();

    /*! Initialize abf sections */
    abfIntervalUsF32 = (float)(1000000.0/samplingRateHz);
    voltageDecimationRatio = 1;
    abfIntervalUsF32V = abfIntervalUsF32*(float)voltageDecimationRatio;
...
void EpisodicAbfDataWriterConsumer::writeSynchInfo(ABF * abf) {
    int maxLength = 0;
